...
function addBidToHistory(bidder, amount) {
    ...
    listItem.textContent = `${bidder} bid ₹${amount}`;
    historyList.prepend(listItem);
}

function startFakeBidding(auctionId) {
    setInterval(() => {
        const currentBidElement = document.getElementById('currentBid');
        const currentBid = parseInt(currentBidElement.textContent);
        const fakeBidders = ['John', 'Alice', 'Bob', 'Emma'];
        const fakeBidder = fakeBidders[Math.floor(Math.random() * fakeBidders.length)];
        const fakeBidAmount = currentBid + Math.floor(Math.random() * 100) + 50;
        
        currentBidElement.textContent = fakeBidAmount;
        addBidToHistory(fakeBidder, fakeBidAmount);
...
function updateCountdown() {
    document.querySelectorAll('.countdown').forEach(el => {
        ...
        if (seconds >= 0) {
            ...
            el.innerHTML = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${remainingSeconds.toString().padStart(2, '0')}`;
        }
    });
}
